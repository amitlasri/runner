...
# For each failed execution create log with all the system calls ran by the command.
call_trace(){
export CALL_TRACE_FILE=./call_trace-${SCRIPT_START_TIME}.log
strace -ttT -o ${CALL_TRACE_FILE} ${COMMAND} >/dev/null 2>&1
}

# For each failed execution create stdout and stderr log files with output of the command.
log_trace(){
...
if [[ ! -f ./return_codes.txt ]] ; then
    ...
    # Count occurrences of each line and print to the same file the return code and the count.
...
# Check if --failed-count is not sent as argument to script. in case that only -c exists.
...
# Running until getting to limit of count or failed-count parameters.
